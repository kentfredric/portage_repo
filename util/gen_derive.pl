...
my %roots = (
  # source => '6f7c2ba0c7d03f041cda9bda8f8cda731b3b8a37',
  #  dest  => '0f5512976aab6ec0ebe0e634a7a59b8acec50c49',
);

my %whitelists = (
    sources => [
        qw(
          examples src tests .gitignore Cargo.lock Cargo.toml rustfmt.toml
          util/rustflags.sh
          )
    ],
    tools => [
...
if( $ENV{SOURCE_COMMIT} ) {
  $roots{source} = $ENV{SOURCE_COMMIT};
}
if ( $ENV{DEST_COMMIT}) {
  $roots{dest} = $ENV{DEST_COMMIT};
}
if ( $ENV{DEST} ) {
...
if ( not exists $roots{dest} ) {
  system( 'git', 'checkout', '-f', '-l', '--orphan', $dest_branch ) == 0
    or die "Can't invoke checkout, $!, $?";
} else {
  system( 'git', 'checkout', '-f', '-l', '-B', $dest_branch, $roots{dest} ) == 0
    or die "Can't invoke checkout, $!, $?";
}
if( exists $roots{source} ) {
  while(@{ $original->{order}}) {
    my $item = pop @{$original->{order}};
    last if $item eq $roots{source};
  }
  if ( exists $roots{dest} ) {
    $parent_trans->{$roots{source}} = $roots{dest};
  }
}
